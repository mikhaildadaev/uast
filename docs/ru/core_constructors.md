...
## NewComment
Создаёт новый экземпляр оператора COMMENT. Принимает текст комментария и возвращает оператор, который можно настроить с помощью `OnColumn` или `Is`.
```go
...
## NewCreate
Создаёт новый экземпляр оператора CREATE. Принимает источник `Index/Schema/Table/View` и возвращает оператор, который можно настроить с помощью методов `Columns`, `Constraints`, `IfNotExists`, `IsReplace`, `IsUnique`, `On`, `Source`.
```go
