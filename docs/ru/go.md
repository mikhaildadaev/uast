...
- **Безопасность на уровне архитектуры** — Трёхуровневая система `Constant` / `Literal` / `Value` предотвращает SQL-инъекции.
- **Высокая производительность** — `sync.Pool` для переиспользования контекста, ~216 нс/оп для простых запросов в мутабельном режиме (до 2 раз быстрее иммутабельного). Иммутабельный режим работает на ~450 нс/оп, гарантируя потокобезопасность.
