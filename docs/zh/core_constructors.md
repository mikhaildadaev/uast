...
::: info **关于**
此页面涵盖以下语句构造函数：`NewAlter`、`NewComment`、`NewCreate`、`NewDelete`、`NewDrop`、`NewInsert`、`NewSelect`、`NewTruncate`、`NewUpdate`。每个构造函数创建一个新的语句实例，可使用方法进行配置，并通过 `Build()` 编译为 SQL。
:::

## NewAlter
创建一个新的 ALTER 语句实例。接受 `Index/Schema/Table/View` 类型的源，并返回一个可以通过 `AddColumns`、`AddConstraints`、`DropColumns`、`DropConstraints`、`RenameTo`、`RenameColumn`、`RenameConstraint`、`SetColumns` 方法进行配置的语句。
```go
...
stmtAlterTableRenameTo := uast.NewAlter(uast.NewTable("users", "u")).
    RenameTo("new_users")
...
## NewComment
创建一个新的 COMMENT 语句实例。接受注释文本，并返回一个可以使用 `OnColumn` 或 `Is` 进行配置的语句。
```go
stmtCommentColumn := uast.NewComment(uast.NewTable("users", "u")).OnColumn(uast.Field[int64]("u", "id")).
    Is("text")
...
## NewCreate
创建一个新的 CREATE 语句实例。接受 `Index/Schema/Table/View` 源，并返回一个可以通过 `Columns`、`Constraints`、`IfNotExists`、`IsReplace`、`IsUnique`、`On`、`Source` 方法进行配置的语句。
```go
