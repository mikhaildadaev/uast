...
func Test_SQL_Create(t *testing.T) {
	...
	t.Run("Table", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			switch supportDialect {
			case DialectMariaDB:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` SIGNED AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR NOT NULL)", "CREATE TABLE")
			case DialectMsSQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE [test] ([id] BIGINT IDENTITY(1,1) PRIMARY KEY, [name] NVARCHAR NOT NULL)", "CREATE TABLE")
			case DialectMySQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` SIGNED AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR NOT NULL)", "COLUMN NAME")
			case DialectPostgreSQL:
