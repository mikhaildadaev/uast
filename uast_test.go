	...
	t.Run("Table", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			stmtCreate := NewCreate(NewTable("test", "")).
				...
				Field(
					Test.Column.ID.Expr(),
					Test.Column.Name.Expr(),
				)
			sqlCreateQuery, sqlCreateArguments, err := sql.Build(stmtCreate)
			switch supportDialect {
			case DialectMariaDB:
				//assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` BIGINT AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR NOT NULL)", "CREATE TABLE")
			case DialectMsSQL:
				//assertContains(t, sqlCreateQuery, "CREATE TABLE [test] ([id] BIGINT IDENTITY(1,1) PRIMARY KEY, [name] NVARCHAR NOT NULL)", "CREATE TABLE")
			case DialectMySQL:
				//assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` BIGINT AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR NOT NULL)", "COLUMN NAME")
			case DialectPostgreSQL:
				//assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test" ("id" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "name" VARCHAR NOT NULL)`, "CREATE TABLE")
			case DialectSQLite:
				//assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT NOT NULL)`, "CREATE TABLE")
			}
			t.Logf("\nerr: [%s] \nsdn: %s \nsqa: [%s] \nsql: [%s]", err, sqlCreateArguments, supportDialect.name, sqlCreateQuery)
		})
	})
