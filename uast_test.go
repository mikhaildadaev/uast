		...
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			stmtCreate := NewCreate(Test.Table).
				Constraints(
					...
					NewForeignKey("fk_test_data", Data.Table,
						[]SourceBase{Test.Column.DataID},
						[]SourceBase{Data.Column.ID},
						OnDelete(Cascade()),
						OnUpdate(Restrict()),
					),
			...
			switch supportDialect {
			case DialectMariaDB:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` SIGNED AUTO_INCREMENT, `name` VARCHAR NOT NULL, CONSTRAINT `pk_orders` PRIMARY KEY(`id`), CONSTRAINT `uk_orders` UNIQUE(`name`), CONSTRAINT `fk_test_data` FOREIGN KEY(`data_id`)REFERENCES`data`(`id`) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT `ck_id` CHECK(`t`.`id` > ?))", "CREATE TABLE")
			case DialectMsSQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE [test] ([id] BIGINT IDENTITY(1,1), [name] NVARCHAR NOT NULL, CONSTRAINT [pk_orders] PRIMARY KEY([id]), CONSTRAINT [uk_orders] UNIQUE([name]), CONSTRAINT [fk_test_data] FOREIGN KEY([data_id])REFERENCES[data]([id]) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT [ck_id] CHECK([t].[id] > @p1))", "CREATE TABLE")
			case DialectMySQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` SIGNED AUTO_INCREMENT, `name` VARCHAR NOT NULL, CONSTRAINT `pk_orders` PRIMARY KEY(`id`), CONSTRAINT `uk_orders` UNIQUE(`name`), CONSTRAINT `fk_test_data` FOREIGN KEY(`data_id`)REFERENCES`data`(`id`) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT `ck_id` CHECK(`t`.`id` > ?))", "COLUMN NAME")
			case DialectPostgreSQL:
				assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test" ("id" BIGINT GENERATED BY DEFAULT AS IDENTITY, "name" VARCHAR NOT NULL, CONSTRAINT "pk_orders" PRIMARY KEY("id"), CONSTRAINT "uk_orders" UNIQUE("name"), CONSTRAINT "fk_test_data" FOREIGN KEY("data_id")REFERENCES"data"("id") ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT "ck_id" CHECK("t"."id" > $1))`, "CREATE TABLE")
			case DialectSQLite:
				assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test" ("id" INTEGER AUTOINCREMENT, "name" TEXT NOT NULL, CONSTRAINT "pk_orders" PRIMARY KEY("id"), CONSTRAINT "uk_orders" UNIQUE("name"), CONSTRAINT "fk_test_data" FOREIGN KEY("data_id")REFERENCES"data"("id") ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT "ck_id" CHECK("t"."id" > ?))`, "CREATE TABLE")
			}
			t.Logf("\nerr: [%s] \nsdn: %s \nsqa: [%s] \nsql: [%s]", err, sqlCreateArguments, supportDialect.name, sqlCreateQuery)
		})
	})
