...
func Test_SQL_Create(t *testing.T) {
	t.Run("Index", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			case DialectMariaDB:
				...
				assertContains(t, sqlCreateQuery, "CREATE UNIQUE INDEX IF NOT EXISTS `idx_test`", "CREATE INDEX")
			case DialectMsSQL:
				//assertContains(t, sqlCreateQuery, "CREATE UNIQUE INDEX [idx_test] ON [test] ([string], [number])", "CREATE INDEX")
				assertContains(t, sqlCreateQuery, "CREATE UNIQUE INDEX [idx_test]", "CREATE INDEX")
			case DialectMySQL:
				//assertContains(t, sqlCreateQuery, "CREATE UNIQUE INDEX `idx_test` ON `test` (`string`, `number`)", "CREATE INDEX")
				assertContains(t, sqlCreateQuery, "CREATE UNIQUE INDEX `idx_test`", "CREATE INDEX")
			case DialectPostgreSQL:
	...
	t.Run("Table", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			switch supportDialect {
			case DialectMariaDB:
				//assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test`", "CREATE TABLE")
				//assertContains(t, sqlCreateQuery, "`id` BIGINT AUTO_INCREMENT PRIMARY KEY", "COLUMN ID")
				//assertContains(t, sqlCreateQuery, "`name` VARCHAR NOT NULL", "COLUMN NAME")
			case DialectMsSQL:
				//assertContains(t, sqlCreateQuery, "CREATE TABLE [test]", "CREATE TABLE")
				//assertContains(t, sqlCreateQuery, "[id] BIGINT IDENTITY(1,1) PRIMARY KEY", "COLUMN ID")
				//assertContains(t, sqlCreateQuery, "[name] NVARCHAR NOT NULL", "COLUMN NAME")
			case DialectMySQL:
				//assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test`", "CREATE TABLE")
				//assertContains(t, sqlCreateQuery, "`id` BIGINT AUTO_INCREMENT PRIMARY KEY", "COLUMN ID")
				//assertContains(t, sqlCreateQuery, "`name` VARCHAR NOT NULL", "COLUMN NAME")
			case DialectPostgreSQL:
				//assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test"`, "CREATE TABLE")
				//assertContains(t, sqlCreateQuery, `"id" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`, "COLUMN ID")
				//assertContains(t, sqlCreateQuery, `"name" VARCHAR NOT NULL`, "COLUMN NAME")
			case DialectSQLite:
				//assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test"`, "CREATE TABLE")
				//assertContains(t, sqlCreateQuery, `"id" INTEGER PRIMARY KEY AUTOINCREMENT`, "COLUMN ID")
				//assertContains(t, sqlCreateQuery, `"name" TEXT NOT NULL`, "COLUMN NAME")
			}
			t.Logf("\nerr: [%s] \nsdn: %s \nsqa: [%s] \nsql: [%s]", err, sqlCreateArguments, supportDialect.name, sqlCreateQuery)
		})
	})
