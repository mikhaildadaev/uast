...
func Test_SQL_Create(t *testing.T) {
	t.Run("Index", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			defer sql.Close()
			stmtCreate := NewCreate(NewIndex("idx_test", Test.Table).Unique()).
				IfNotExists().
				On(Test.Table).
				Columns(
					Test.Column.String,
	...
	t.Run("Table", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			stmtCreate := NewCreate(NewTable("test", "")).
				Constraints(
					NewPrimaryKey("pk_orders", "id"),
					NewUnique("uk_orders", "name"),
			...
			switch supportDialect {
			case DialectMariaDB:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` SIGNED AUTO_INCREMENT, `name` VARCHAR NOT NULL, CONSTRAINT `pk_orders` PRIMARY KEY(`id`), CONSTRAINT `uk_orders` UNIQUE(`name`), CONSTRAINT `ck_id` CHECK(`t`.`id` > ?))", "CREATE TABLE")
			case DialectMsSQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE [test] ([id] BIGINT IDENTITY(1,1), [name] NVARCHAR NOT NULL, CONSTRAINT [pk_orders] PRIMARY KEY([id]), CONSTRAINT [uk_orders] UNIQUE([name]), CONSTRAINT [ck_id] CHECK([t].[id] > @p1))", "CREATE TABLE")
			case DialectMySQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `test` (`id` SIGNED AUTO_INCREMENT, `name` VARCHAR NOT NULL, CONSTRAINT `pk_orders` PRIMARY KEY(`id`), CONSTRAINT `uk_orders` UNIQUE(`name`), CONSTRAINT `ck_id` CHECK(`t`.`id` > ?))", "COLUMN NAME")
			case DialectPostgreSQL:
				assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test" ("id" BIGINT GENERATED BY DEFAULT AS IDENTITY, "name" VARCHAR NOT NULL, CONSTRAINT "pk_orders" PRIMARY KEY("id"), CONSTRAINT "uk_orders" UNIQUE("name"), CONSTRAINT "ck_id" CHECK("t"."id" > $1))`, "CREATE TABLE")
			case DialectSQLite:
				assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "test" ("id" INTEGER AUTOINCREMENT, "name" TEXT NOT NULL, CONSTRAINT "pk_orders" PRIMARY KEY("id"), CONSTRAINT "uk_orders" UNIQUE("name"), CONSTRAINT "ck_id" CHECK("t"."id" > ?))`, "CREATE TABLE")
			}
			t.Logf("\nerr: [%s] \nsdn: %s \nsqa: [%s] \nsql: [%s]", err, sqlCreateArguments, supportDialect.name, sqlCreateQuery)
		})
	})
...
func init() {
	...
	Test.Column.UserID = NewColumn[int64]("user_id", Test.Table, TypeBigInt)
