...
func Test_SQL_Alter(t *testing.T) {
	...
	t.Run("Schema_RenameTo", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			sql := NewSQL(WithDialect(supportDialect))
			defer sql.Close()
	...
	t.Run("Table_RenameConstraint", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			switch supportDialect {
			case DialectMariaDB:
				assertContains(t, sqlAlterQuery, "ALTER TABLE `users` RENAME CONSTRAINT `ck_users_number` TO `new_name`", "RENAME CONSTRAINT")
			case DialectMsSQL:
	...
	t.Run("Table_RenameTo", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			sql := NewSQL(WithDialect(supportDialect))
			defer sql.Close()
			stmtAlter := NewAlter(Test.Table.User).
	...
	t.Run("Table_Set", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			sql := NewSQL(WithDialect(supportDialect))
			defer sql.Close()
			stmtAlter := NewAlter(Test.Table.User).
	...
	t.Run("View_RenameTo", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			sql := NewSQL(WithDialect(supportDialect))
			defer sql.Close()
...
func Test_SQL_Create(t *testing.T) {
	...
	t.Run("Table", func(t *testing.T) {
		testAllDialects(t, func(t *testing.T, supportDialect *SupportDialect) {
			...
			switch supportDialect {
			case DialectMariaDB:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `users` (`id` SIGNED AUTO_INCREMENT, `name` VARCHAR NOT NULL, CONSTRAINT `ck_users_number` CHECK(`u`.`number` > ?), CONSTRAINT `fk_users_orders` FOREIGN KEY(`order_id`, `name`) REFERENCES `orders`(`id`, `string`) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT `pk_users_id` PRIMARY KEY(`id`), CONSTRAINT `un_users_name` UNIQUE(`name`))", "CREATE TABLE")
			case DialectMsSQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE [users] ([id] BIGINT IDENTITY(1,1), [name] NVARCHAR NOT NULL, CONSTRAINT [ck_users_number] CHECK([u].[number] > @p1), CONSTRAINT [fk_users_orders] FOREIGN KEY([order_id], [name]) REFERENCES [orders]([id], [string]) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT [pk_users_id] PRIMARY KEY([id]), CONSTRAINT [un_users_name] UNIQUE([name]))", "CREATE TABLE")
			case DialectMySQL:
				assertContains(t, sqlCreateQuery, "CREATE TABLE IF NOT EXISTS `users` (`id` SIGNED AUTO_INCREMENT, `name` VARCHAR NOT NULL, CONSTRAINT `ck_users_number` CHECK(`u`.`number` > ?), CONSTRAINT `fk_users_orders` FOREIGN KEY(`order_id`, `name`) REFERENCES `orders`(`id`, `string`) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT `pk_users_id` PRIMARY KEY(`id`), CONSTRAINT `un_users_name` UNIQUE(`name`))", "CREATE TABLE")
			case DialectPostgreSQL:
			...
			case DialectSQLite:
				assertContains(t, sqlCreateQuery, `CREATE TABLE IF NOT EXISTS "users" ("id" INTEGER AUTOINCREMENT, "name" TEXT NOT NULL, CONSTRAINT "ck_users_number" CHECK("u"."number" > ?), CONSTRAINT "fk_users_orders" FOREIGN KEY("order_id", "name") REFERENCES "orders"("id", "string") ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT "pk_users_id" PRIMARY KEY("id"), CONSTRAINT "un_users_name" UNIQUE("name"))`, "CREATE TABLE")
			}
			t.Logf("\nerr: [%s] \nsdn: %s \nsqa: [%s] \nsql: [%s]", err, sqlCreateArguments, supportDialect.name, sqlCreateQuery)
		})
	})
...
func init() {
	...
	Test.Table.Orders.UserID = NewColumn[int64]("user_id", Test.Table.Order, TypeBigInt)
	...
	Test.Table.Users.OrderID = NewColumn[int64]("order_id", Test.Table.User, TypeBigInt)
	...
	Test.Foreign.UsersOrders = NewForeignKey("fk_users_orders", Test.Table.Order, Cascade(), Restrict(), Relation(Test.Table.Users.OrderID, Test.Table.Orders.ID), Relation(Test.Table.Users.Name, Test.Table.Orders.String))
